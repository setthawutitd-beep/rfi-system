...
export default function RFIModal({ rfi, open, onClose, currentUser, currentRole, onAction, onComment, onToast }: Props) {
  ...
  // เก็บ currentNodeId แยก เพื่อไม่ขึ้นกับ rfi prop ที่อาจ stale
  const [activeNodeId, setActiveNodeId] = useState<string | null>(null)

  const fileRef = useRef<HTMLInputElement>(null)

  // ─── Load flow data ───────────────────────────────────────────
  const loadFlow = useCallback(async (flowTemplateId: string, currentNodeId: string | null | undefined, rfiId: string) => {
    setFlowLoaded(false)
    setFlowNodes([])
    setFlowEdges([])
    setFlowLevels([])
    ...
    // โหลด flow structure + role groups พร้อมกัน
    const [{ data: n }, { data: e }, { data: rg }] = await Promise.all([
    ...
    const nodes = (n || []) as FlowNode[]
    const edges = (e || []) as FlowEdge[]
    setFlowNodes(nodes)
    setFlowEdges(edges)
    setRoleGroups(rg || [])

    // หา currentNode จาก nodes ที่เพิ่ง load มา
    const found = nodes.find(x => x.id === currentNodeId) || null
    setCurrentNode(found)
    setFlowLoaded(true)

    // โหลดส่วนที่เหลือแบบ async ไม่บล็อก UI
    const nodeIds = nodes.map(x => x.id)
    if (nodeIds.length === 0) return

    const [{ data: l }, { data: d }, { data: subs }, { data: approvals }] = await Promise.all([
      supabase.from('flow_node_levels').select('*').in('node_id', nodeIds).order('sort_order'),
      supabase.from('rfi_doc_requests').select('*').eq('rfi_id', rfiId)
        .not('status', 'eq', 'rejected').order('sort_order').order('rev_no'),
      supabase.from('rfi_doc_submissions').select('*, profile:submitted_by(name, color)').eq('rfi_id', rfiId),
      supabase.from('rfi_approvals').select('*, user:user_id(name, color, avatar)').eq('rfi_id', rfiId),
    ])

    setFlowLevels((l || []) as FlowLevel[])
    setDocSubmissions(subs || [])
    setRfiApprovals(approvals || [])
    setDocRequests((d || []) as DocRequest[])
  ...
  useEffect(() => {
    ...
    if (!rfi.flow_template_id) {
      // ไม่มี flow — reset แล้วแสดงปุ่ม assign
      setFlowLoaded(true)
      setFlowNodes([])
      setFlowEdges([])
      setFlowLevels([])
      ...
      // โหลด role groups สำหรับปุ่ม assign
      supabase.from('role_groups').select('id, name, label, color')
        .then(({ data }) => setRoleGroups(data || []))
      return
    }
  ...
  const getNextNode = (condition = 'yes') => {
    const edge = flowEdges.find(e => e.source_id === effectiveNodeId && e.condition === condition)
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, docRequestId?: string) => {
    ...
    const path = `${rfi.id}/${Date.now()}.${ext}`
    const { error } = await supabase.storage.from('rfi-attachments').upload(path, file)
    ...
    await supabase.from('rfi_doc_submissions').insert({
      rfi_id: rfi.id, doc_request_id: docRequestId || null,
      node_id: effectiveNodeId, filename: file.name,
      file_url: url.publicUrl, file_size: file.size,
      mime_type: file.type, submitted_by: currentUser.id, status: 'pending',
    })
  ...
  const renderTimeline = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {flowNodes.map((node, i) => {
        const nodeIdx = flowNodes.findIndex(n => n.id === effectiveNodeId)
        const isPast = i < nodeIdx
        const isCurr = node.id === effectiveNodeId
  ...
  const renderActionPanel = () => {
    if (!flowLoaded) return (
      <div style={apStyle}>
        <div style={{ fontSize: 11.5, color: 'var(--text3)', display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ animation: 'spin 1s linear infinite', display: 'inline-block' }}>⟳</span> กำลังโหลด...
        </div>
      </div>
    )
    ...
    // ── มี flow แต่ยังไม่รู้ current node ──
    if (!currentNode) return (
      <div style={apStyle}>
        <div style={{ fontSize: 11.5, color: 'var(--text3)' }}>
          {effectiveNodeId ? 'กำลังหา Node ปัจจุบัน...' : 'ไม่พบ Node ปัจจุบันใน Flow'}
        </div>
      </div>
    )

    const nodeApprovals = rfiApprovals.filter(a => a.node_id === effectiveNodeId)
                ...
                {showAddDoc ? (
                  <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: 10, marginBottom: 10 }}>
                    <input className="input" placeholder="ชื่อเอกสาร *" value={newDoc.label}
                      onChange={e => setNewDoc(p => ({ ...p, label: e.target.value }))} style={{ marginBottom: 6 }} />
                    <input className="input" placeholder="คำอธิบาย" value={newDoc.description}
                      onChange={e => setNewDoc(p => ({ ...p, description: e.target.value }))} style={{ marginBottom: 6 }} />
    ...
    if (currentNodeLevels.some(l => l.action_type === 'attach')) {
      ...
      const submittedCount = requiredDocs.filter(d => d.status === 'submitted' || d.status === 'approved').length
            ...
            <>
                  ...
                  <div key={doc.id} style={{
                    ...
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 4 }}>
                      {(doc.responsible_ids || []).map(id => {
                        ...
                        if (!rg) return null
                        return <span key={id} style={{ fontSize: 9, padding: '1px 6px', borderRadius: 3, background: `${rg.color}18`, border: `1px solid ${rg.color}`, color: rg.color }}>{rg.label}</span>
                      })}
                      {(doc.responsible_ids || []).length === 0 && <span style={{ fontSize: 9, color: 'var(--text3)' }}>ทุกคน</span>}
                    </div>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
                        {doc.status !== 'approved' && (
                          <label style={{
                            ...
                            <input type="file" style={{ display: 'none' }} onChange={async e => {
                              if (mySubmission) await supabase.from('rfi_doc_submissions').delete().eq('id', mySubmission.id)
                              await handleFileUpload(e, doc.id)
                              await supabase.from('rfi_doc_requests').update({ status: 'submitted' }).eq('id', doc.id)
                              const [{ data: nd }, { data: ns }] = await Promise.all([
                                supabase.from('rfi_doc_requests').select('*').eq('rfi_id', rfi.id).not('status', 'eq', 'rejected').order('sort_order').order('rev_no'),
                                supabase.from('rfi_doc_submissions').select('*, profile:submitted_by(name, color)').eq('rfi_id', rfi.id),
                              ])
                              setDocRequests(nd || [])
                              setDocSubmissions(ns || [])
                            }} />
                          </label>
                        )}
                        {mySubmission && doc.status !== 'approved' && (
                          <button onClick={async () => {
                            ...
                            await supabase.from('rfi_doc_requests').update({ status: 'pending' }).eq('id', doc.id)
                            const [{ data: nd }, { data: ns }] = await Promise.all([
                              supabase.from('rfi_doc_requests').select('*').eq('rfi_id', rfi.id).not('status', 'eq', 'rejected').order('sort_order').order('rev_no'),
                              supabase.from('rfi_doc_submissions').select('*, profile:submitted_by(name, color)').eq('rfi_id', rfi.id),
                            ])
                          ...
                          }} style={{ padding: '4px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 11.5, background: 'rgba(240,80,96,0.1)', border: '1px solid var(--red)', color: 'var(--red)' }}>🗑 ลบ</button>
                        )}
                        {doc.rev_no > 0 && (
                          <span style={{ fontSize: 10, padding: '4px 8px', borderRadius: 6, background: 'rgba(245,197,66,0.15)', border: '1px solid var(--yellow)', color: 'var(--yellow)' }}>Rev.{doc.rev_no}</span>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ height: 4, background: 'var(--surface3)', borderRadius: 2, overflow: 'hidden', marginBottom: 10 }}>
                  <div style={{ height: '100%', borderRadius: 2, background: allDone ? 'var(--green)' : 'var(--accent)', width: `${Math.min(100, (submittedCount / Math.max(requiredDocs.length, 1)) * 100)}%`, transition: 'width 0.3s' }} />
                </div>
                <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}
                  disabled={!allDone || actionLoading}
                  onClick={() => handleAdvance('yes')}>
                  {actionLoading ? '⟳ กำลังส่ง...' : allDone ? '✓ ส่งเอกสารไปขั้นตอนถัดไป →' : `รอแนบเอกสารบังคับให้ครบก่อน (${submittedCount}/${requiredDocs.length})`}
                </button>
              </div>
            </>
          )}
    ...
    if (currentNodeLevels.some(l => l.action_type === 'review')) {
      const nodeApprovals2 = rfiApprovals.filter(a => a.node_id === effectiveNodeId)
      const myApproval2 = nodeApprovals2.find(a => a.user_id === currentUser?.id)
      const approvedCount = nodeApprovals2.filter(a => a.action === 'approve').length
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={apStyle}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              {(reviewLevelAny?.assignee_ids || []).map(id => {
                ...
                return (
                  <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 8px', borderRadius: 6, marginBottom: 4, background: isMe ? `${rg.color}18` : 'var(--surface2)', border: `1px solid ${isMe ? rg.color : 'var(--border)'}` }}>
          ...
          {reviewLevel && !myApproval2 && (
            <div style={apStyle}>
              ...
              {docRequests.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
                  {docRequests.map(doc => {
                    ...
                    return (
                      <div key={doc.id} style={{ background: 'var(--surface3)', borderRadius: 8, padding: '10px 12px', border: `1px solid ${rev.status === 'approved' ? 'var(--green)' : rev.status === 'rejected' ? 'var(--red)' : 'var(--border)'}` }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                          <span style={{ fontSize: 9, color: doc.required ? 'var(--red)' : 'var(--green)' }}>●</span>
                          <span style={{ fontSize: 12, fontWeight: 600, flex: 1 }}>{doc.label}</span>
                          {doc.rev_no > 0 && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: 'rgba(245,197,66,0.15)', color: 'var(--yellow)', border: '1px solid var(--yellow)' }}>Rev.{doc.rev_no}</span>}
                        </div>
                        {subs.length > 0 ? subs.map(sub => (
                          <a key={sub.id} href={sub.file_url} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10.5, color: 'var(--accent2)', textDecoration: 'none', marginBottom: 3 }}>
                            <span>📄</span>
                          ...
                          <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 8 }}>{doc.required ? '⚠️ ยังไม่มีไฟล์' : '— ไม่มีไฟล์ (optional)'}</div>
                        )}
                        <div style={{ display: 'flex', gap: 5, marginTop: 6 }}>
                          {(['approved', 'rejected', 'skip'] as const).map(s => (
                            <button key={s} onClick={() => setDocReviews(prev => ({ ...prev, [doc.id]: { ...prev[doc.id], status: s, note: prev[doc.id]?.note || '' } }))} style={{
                              flex: 1, padding: '4px', borderRadius: 5, cursor: 'pointer', fontSize: 10.5, fontWeight: rev.status === s ? 700 : 400,
                              background: rev.status === s ? (s === 'approved' ? 'rgba(62,207,142,0.2)' : s === 'rejected' ? 'rgba(240,80,96,0.2)' : 'var(--surface2)') : 'var(--surface2)',
                              border: `1px solid ${rev.status === s ? (s === 'approved' ? 'var(--green)' : s === 'rejected' ? 'var(--red)' : 'var(--border2)') : 'var(--border)'}`,
                              color: rev.status === s ? (s === 'approved' ? 'var(--green)' : s === 'rejected' ? 'var(--red)' : 'var(--text2)') : 'var(--text3)',
                            }}>{s === 'approved' ? '✓ ผ่าน' : s === 'rejected' ? '✕ ไม่ผ่าน' : '— ข้าม'}</button>
                          ))}
                        </div>
                        {rev.status === 'rejected' && (
                          <input className="input" placeholder="เหตุผล (บังคับ) *" value={rev.note} style={{ marginTop: 6, fontSize: 11 }}
                            onChange={e => setDocReviews(prev => ({ ...prev, [doc.id]: { ...prev[doc.id], note: e.target.value } }))} />
                        )}
                      </div>
                    )
                  })}
              ...
              {(() => {
                ...
                const requiredNotReviewed = docRequests.filter(d => d.required && docReviews[d.id]?.status === 'skip').length
                return (
                  <div>
                    {requiredNotReviewed > 0 && <div style={{ fontSize: 11, color: 'var(--yellow)', marginBottom: 6 }}>⚠️ ยังมีเอกสารบังคับที่ยังไม่ได้ Review {requiredNotReviewed} อัน</div>}
                    {rejectMissingNote && <div style={{ fontSize: 11, color: 'var(--red)', marginBottom: 6 }}>⚠️ กรุณาใส่เหตุผลสำหรับเอกสารที่ไม่ผ่าน</div>}
                    <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}
                      ...
                      onClick={async () => {
                        ...
                        const rejected2 = docRequests.filter(d => docReviews[d.id]?.status === 'rejected')
                        for (const doc of approved) await supabase.from('rfi_doc_requests').update({ status: 'approved' }).eq('id', doc.id)
                        for (const doc of rejected2) {
                          await supabase.rpc('reject_doc_and_create_revision', { p_doc_id: doc.id, p_rejected_by: currentUser?.id, p_reject_note: docReviews[doc.id].note })
                        }
                        await supabase.from('rfi_approvals').insert({
                          rfi_id: rfi.id, node_id: effectiveNodeId, level_id: reviewLevel.id, user_id: currentUser?.id,
                          action: rejected2.length > 0 ? 'reject' : 'approve',
                          note: rejected2.length > 0 ? `ไม่ผ่าน ${rejected2.length} อัน` : `ผ่านทั้งหมด ${approved.length} อัน`,
                        })
                        await supabase.from('rfi_activity_log').insert({
                          rfi_id: rfi.id, node_id: effectiveNodeId,
                          action_type: rejected2.length > 0 ? 'review_fail' : 'review_pass', actor_id: currentUser?.id,
                          note: rejected2.length > 0 ? `Reject ${rejected2.length} อัน` : 'อนุมัติเอกสารทั้งหมด',
                        })
                        setDocReviews({})
                        setActionLoading(false)
                        onToast(rejected2.length > 0 ? 'ส่งกลับแก้ไข' : 'อนุมัติแล้ว', rejected2.length > 0 ? `${rejected2.length} เอกสารต้องแนบใหม่` : 'ผ่านทั้งหมด', rejected2.length > 0 ? 'error' : 'success')
                        handleAdvance(rejected2.length > 0 ? 'no' : 'yes')
                      }}>
          ...
          {myApproval2 && (
            <div style={{ ...apStyle, borderColor: myApproval2.action === 'approve' ? 'var(--green)' : 'var(--red)' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: myApproval2.action === 'approve' ? 'var(--green)' : 'var(--red)' }}>
                {myApproval2.action === 'approve' ? '✓ คุณอนุมัติแล้ว' : '✕ คุณส่งกลับแล้ว'}
              </div>
              {myApproval2.note && <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 3 }}>{myApproval2.note}</div>}
            </div>
          )}
      ...
      <div style={apStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {(decisionLevel.decision_options || []).length > 0
            ...
            : (
              <div style={{ display: 'flex', gap: 6 }}>
                <button className="btn btn-primary btn-sm" style={{ flex: 1, justifyContent: 'center' }} onClick={() => handleAdvance('yes')} disabled={actionLoading}>ใช่</button>
                <button className="btn btn-success btn-sm" style={{ flex: 1, justifyContent: 'center' }} onClick={() => handleAdvance('no')} disabled={actionLoading}>ไม่ใช่</button>
              </div>
            )
          }
        </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <span onClick={onClose} style={{ marginLeft: 'auto', width: 28, height: 28, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>✕</span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 280px', gap: 16 }}>
            ...
            {activeTab === 'history' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {activityLog.length === 0 ? (
                  <div style={{ fontSize: 12, color: 'var(--text3)', textAlign: 'center', padding: '20px 0' }}>ยังไม่มีประวัติ</div>
                ) : activityLog.map(log => (
                  <div key={log.id} style={{ display: 'flex', gap: 10, padding: '8px 10px', background: 'var(--surface2)', borderRadius: 8, borderLeft: `3px solid ${log.action_type === 'review_pass' ? 'var(--green)' : log.action_type === 'review_fail' ? 'var(--red)' : log.action_type === 'advance' ? 'var(--accent2)' : 'var(--border2)'}` }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: log.actor?.color || 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#0a0d12' }}>
                      {log.actor?.avatar || log.actor?.name?.[0] || '?'}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        <span style={{ fontSize: 12, fontWeight: 600 }}>{log.actor?.name}</span>
                        <span style={{ fontSize: 10, padding: '1px 5px', borderRadius: 3, background: 'var(--surface3)', color: 'var(--text3)' }}>{log.action_type}</span>
                      </div>
                      {log.note && <div style={{ fontSize: 11.5, color: 'var(--text2)' }}>{log.note}</div>}
                      <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2, fontFamily: 'IBM Plex Mono, monospace' }}>{new Date(log.created_at).toLocaleString('th-TH')}</div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {docRequests.length === 0 ? (
                  ...
                  <div key={doc.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', marginBottom: 6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      ...
                      <span style={{ marginLeft: 'auto', fontSize: 10, padding: '1px 6px', borderRadius: 4, background: doc.status === 'pending' ? 'rgba(245,197,66,0.15)' : 'rgba(62,207,142,0.15)', color: doc.status === 'pending' ? 'var(--yellow)' : 'var(--green)', border: `1px solid ${doc.status === 'pending' ? 'var(--yellow)' : 'var(--green)'}` }}>
                    ...
                    {docSubmissions.filter(s => s.doc_request_id === doc.id).map(sub => (
                      <a key={sub.id} href={sub.file_url} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 8px', background: 'var(--surface3)', borderRadius: 6, marginTop: 4, fontSize: 11, color: 'var(--accent2)', textDecoration: 'none', border: '1px solid var(--border)' }}>
                        <span>📄</span>
                        <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sub.filename}</span>
                        <span style={{ fontSize: 9, color: 'var(--text3)', flexShrink: 0 }}>{sub.profile?.name}</span>
                    ...
                    : (rfi.comments || []).map(c => (
                      <div key={c.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 9.5, color: 'var(--text3)', marginLeft: 'auto' }}>{new Date(c.created_at).toLocaleDateString('th-TH')}</span>
                        </div>
          ...
          <div style={{ borderLeft: '1px solid var(--border)', paddingLeft: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 10 }}>Action</div>
            {renderActionPanel()}
          </div>
        </div>
      </div>
